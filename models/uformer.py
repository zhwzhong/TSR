...
class PBVS(nn.Module):
    def __init__(self):
        ...
        self.head = ConvBNReLU2D(1, out_channels=3, kernel_size=7, act='PReLU', padding=3)
...
class NIR(nn.Module):
    def __init__(self):
        ...
        self.rgb = ConvBNReLU2D(3, out_channels=3, kernel_size=7, act='PReLU', padding=3)
        self.nir = ConvBNReLU2D(1, out_channels=3, kernel_size=7, act='PReLU', padding=3)
        self.fuse = ConvBNReLU2D(6, out_channels=3, kernel_size=7, act='PReLU', padding=3)

    def forward(self, sample):
        nir, rgb = sample['lr_up'], sample['img_rgb']
        out = torch.cat((self.rgb(rgb), self.nir(nir)), dim=1)
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.head = NIR() if args.dataset == 'NIR' else PBVS()
        self.net = Uformer(img_size=args.patch_size, embed_dim=args.embed_dim, win_size=args.win_size,
                           token_projection='linear', token_mlp='leff', depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],
                           modulator=True,dd_in=3)

        self.tail = ConvBNReLU2D(in_channels=3, out_channels=1, kernel_size=7, padding=3)
        if args.pre_trained:
            load_checkpoint(self.net, './ckpt/Uformer_B.pth')
    ...
    def forward(self, samples):
        ...
        if h != self.args.patch_size or w != self.args.patch_size:
            out, mask = expand2square(out, factor=128)

        out = self.tail(self.net(out))
        if mask is not None:
            out = torch.masked_select(out, mask.bool()).reshape(1, 1, h, w)
