...
from models.local_arch import AvgPool2d
...
class RestormerLocal(Local_Base, Restormer):
    def __init__(self, *args, train_size=(1, 3, 256, 256), base_size=(480, 640), fast_imp=False, **kwargs):
...
class PBVS(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.head = nn.Sequential(
            ConvBNReLU2D(1, out_channels=3, kernel_size=7, padding=3),
            ConvBNReLU2D(3, out_channels=3, kernel_size=3, act='PReLU', padding=1)
        )
...
class NIR(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.fuse = nn.Sequential(
            ...
            ConvBNReLU2D(embed_dim, out_channels=3, kernel_size=3, act='PReLU', padding=1),
        )
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        if args.test_only and args.tlc_enhance:
            self.net = RestormerLocal(train_size=train_size, base_size=(640, 640))
        else:
            self.net = Restormer()
    ...
    def forward(self, samples):
        if self.args.test_only:
            p2d = (0, 0, 80, 80)
            samples['lr_up'] = F.pad(samples['lr_up'], p2d, 'reflect')
            if self.args.dataset == 'NIR':
                samples['img_rgb'] = F.pad(samples['img_rgb'], p2d, 'reflect')

        out = self.tail(self.net(self.head(samples)))
        out = out if self.args.no_res else out + samples['lr_up']
        return {'img_out': out[:, :, 80: -80, :] if self.args.test_only else out}
